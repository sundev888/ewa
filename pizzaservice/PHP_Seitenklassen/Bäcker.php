...
class Bäcker extends Page
{
    ...
    //    $this->getViewData();
        ...
        echo '<form class="" action="Bäcker.php" method="POST">';
        $fPizzaname = (string) $pizzenarray[$i]->fPizzaname;
        $status = (string) $pizzenarray[$i]->status;
        $pizzaID = (string) $pizzenarray[$i]->ID;
        if($status ==="bestellt"){
         echo <<<HTML
      $fPizzaname : <input type="text" name="pizzaId" value="$pizzaID">
       bestellt <input type="radio" name="status" value="bestellt" checked ="checked">
                     im Ofen <input type="radio" name="status" value="imOfen" >
                     fertig <input type="radio" name="status" value="fertig">
                   <button type="submit" name="test" value="update">update</button>
                     </form>
HTML;
}
...
if($status ==="imOfen"){
            echo <<<HTML
         $fPizzaname : <input type="text" name="pizzaId" value="$pizzaID">
         bestellt <input type="radio" name="status" value="bestellt" >
                        im Ofen <input type="radio" name="status" value="imOfen" checked ="checked">
                        fertig <input type="radio" name="status" value="fertig">
...
if($status ==="fertig"){
            echo <<<HTML
                $fPizzaname :<input type="text" name="pizzaId" value="$pizzaID">
                  bestellt  <input type="radio" name="status" value="bestellt">
                                im Ofen <input type="radio" name="status" value="imOfen">
                                fertig <input type="radio" name="status" value="fertig" checked ="checked">
              <button type="submit" name="test" value="update">update</button>
                  </form>
...
if(isset($_POST['test'])){
  if(isset($_POST['status']) && (isset($_POST['pizzaId']))){
    $statusNeu = $_POST['status'];
    $pizzaID =  $_POST['pizzaId'];
            $sql2 ="UPDATE bestelltepizza SET Status = '$statusNeu' WHERE PizzaID='$pizzaID'";
            mysqli_query($this->connect, $sql2);
          //    echo "status geändert: " .$statusNeu;
...
/*
   ...
   if (isset($_POST['radioRow1'])) {
      $status = $_POST['radioRow1'];
     }

 if($status === "bestellt"){
 echo $status;
}
else if($status === "imOfen"){
echo $status;
}
...
if (isset($_POST['radioRow2'])) {
   $status = $_POST['radioRow2'];
  }
